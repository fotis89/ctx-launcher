...
- For `tool: copilot` workspaces: both `instructions.md` and `.claude/skills/*` are merged into `<workspace-folder>/AGENTS.md` at launch time, so do not flag either as a Copilot incompatibility. The slash-command UX (`/skill-name`) is lost — skills are discoverable as documented workflows but not directly invokable. If a stale `<workspace-folder>/AGENTS.md` exists alongside outdated source files, note it but expect wl to refresh on the next launch.
