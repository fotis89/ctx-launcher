...
- For `tool: copilot` workspaces: `instructions.md` is mirrored to `<workspace-folder>/AGENTS.md` and `.claude/skills/*` is mirrored to `<workspace-folder>/.copilot/skills/*` (loaded by Copilot as a plugin via `--plugin-dir`). Both are regenerated on every launch — don't flag either as a Copilot incompatibility, and don't flag stale `AGENTS.md` or `.copilot/` content (wl refreshes on next launch). The slash-command UX (`/skill-name`) is preserved through Copilot's plugin system.
