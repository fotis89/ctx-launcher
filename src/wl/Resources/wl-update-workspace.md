...
- For `tool: copilot` workspaces: `instructions.md` is mirrored to `<workspace-folder>/AGENTS.md`, and `.claude/skills/*` directories are temporarily registered in `~/.copilot/settings.json`'s `skillDirectories` for the launch (then unregistered). `AGENTS.md` is regenerated on every launch — don't flag it as a Copilot incompatibility or as stale (wl refreshes on next launch). The slash-command UX (`/skill-name`) is preserved through Copilot's skills system.
