...
**2. Make paths portable.** Walk every path you plan to put in `workspace.json` (both `primaryRepo` and each entry in `additionalDirs`) and apply in order:
  ...
  - **Reuse existing path variables.** Read `~/.wl-workspaces/.paths.json` first. If a variable already maps to the right root (e.g. `$REPOS = D:\repos`), use it instead of inventing a new name. Only define a new variable if no existing one fits.
...
1. Create `~/.wl-workspaces/<slug>/workspace.json`:
   ...
   If the proposal envvar-ized any paths (filter 2 in the pre-proposal checklist) *and the variable does not already appear in `.paths.json`*, edit `~/.wl-workspaces/.paths.json` to add each new variable for this PC. Write `$NAME/...` into the JSON fields.
